...
func (l *LegoHatMotorDriver) runToAngle(angle int, method rotationMethod, opts ...RunOption) (done chan struct{}, err error) {
	...
	durationInSeconds := math.Abs((float64(newPosition) - float64(state.position)) / float64(speed))
	timeoutDuration := time.Millisecond * time.Duration((500 + int(math.Ceil(durationInSeconds)*1000)))
	...
	for _, d := range l.devices {
		...
		d.toDevice <- []byte(fmt.Sprintf("port %d ; combi 0 1 0 2 0 3 0 ; pid %d 0 1 f4 0.0027777778 0 5 0 .1 3 ; set ramp %.2f %.2f %.2f 0\r", d.id, d.id, pos, newPosition, durationInSeconds))
